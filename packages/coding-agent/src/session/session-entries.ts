...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	/**
	 * First entry recorded AFTER the branch snapshot a speculative compaction
	 * summarized. A background speculation arms off a prefix snapshot; if the
	 * branch advances (new user/tool turns) before the armed summary is applied,
	 * those post-snapshot entries are covered by neither the summary nor the
	 * remote replacement payload. This id marks where that suffix begins so
	 * buildSessionContext replays it after the replacement history. Absent for
	 * blocking (non-speculative) compactions, which summarize the live branch.
	 */
